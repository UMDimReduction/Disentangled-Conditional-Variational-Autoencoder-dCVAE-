...
class dCVAE(pl.LightningModule):
    ...
    def encode(self, x, condition):
        # Ensure condition is one-hot encoded or has the correct dimensions
        if condition.dim() == 1:
            condition = torch.nn.functional.one_hot(condition, num_classes=self.condition_dim).float()
        condition = condition.view(-1, self.condition_dim)

        # Concatenate the input and the condition
        x = torch.cat((x.view(-1, self.input_dim), condition), dim=1)  # Combine input and condition
    ...
    def decode(self, z, condition):
        ...
        # Concatenate the latent vector and the condition
