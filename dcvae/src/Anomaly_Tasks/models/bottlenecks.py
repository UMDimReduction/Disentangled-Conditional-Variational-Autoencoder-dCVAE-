...
class GeneralBottleneck(nn.Module):
    def __init__(self, latent_dim):
        super(GeneralBottleneck, self).__init__()
...
class IdentityBottleneck(GeneralBottleneck):
    def forward(self, encoded):
...
class VariationalBottleneck(GeneralBottleneck):
...
class ConditionedBottleneck(GeneralBottleneck):
    def __init__(self, latent_dim, sparsity, beta=1.0):
        super().__init__(latent_dim)
...
class CorrelatedBottleneck(GeneralBottleneck):
    ...
    def _loss(self, encoded, latent_code):
        TC_loss = self.sum_squared_error(latent_code, encoded.detach())
        commitment_loss = self.sum_squared_error(encoded, latent_code.detach())
        loss = TC_loss + self.beta * commitment_loss
...
class ConditionalEstimator(torch.autograd.Function):
    @staticmethod
