"""
Building utilities for dCVAE project.
...
def build_ae(model_type, input_shape, anomaly, latent_dim=20, condition_dim=10, beta=4.0):
    """
    Build the autoencoder model based on the type specified.
    
    Args:
        model_type: Type of the model (VAE, dCVAE, etc.)
        input_shape: Shape of the input data.
        anomaly: Whether it's for anomaly detection.
        latent_dim: Dimensionality of the latent space.
        condition_dim: Conditional dimension for models that use conditioning.
        beta: Beta parameter for dCVAE/BetaVAE (controls TC/KL weight).
    
    Returns:
        ae: The built autoencoder model.
    """
    # Adjust latent dimensions for complex models
    latent_dim = 50 if model_type in ['factorvae', 'rfvae'] else latent_dim

    # Map model_type to the correct class
    ae_class = AUTOENCODERS.get(model_type.lower())

    if ae_class is None:
        raise ValueError(f"Unknown model_type: {model_type}. Available: {list(AUTOENCODERS.keys())}")

    # Build model with appropriate parameters
    if model_type == 'dcvae':
        # CORRECTED: dCVAE now takes beta parameter for TC weighting
        ae = ae_class(input_shape, latent_dim=latent_dim, condition_dim=condition_dim, beta=beta)
    elif model_type == 'cvae':
        ae = ae_class(input_shape, latent_dim=latent_dim, condition_dim=condition_dim)
    elif model_type == 'betavae':
        ae = ae_class(input_shape, latent_dim=latent_dim, beta=beta)
    else:
...
def load_ae_from_checkpoint(model_type, input_shape, anomaly, checkpoint_path, beta=4.0):
    """
    ...
    Args:
        ...
        beta: Beta parameter (for dCVAE/BetaVAE).
    
    Returns:
        model: The loaded autoencoder model.
    """
    checkpoint = torch.load(checkpoint_path)
    model = build_ae(model_type, input_shape, anomaly, beta=beta)
